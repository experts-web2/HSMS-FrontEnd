  ...
  implements OnInit
{
  ...
  paymentType: Array<{ id: number; label: string }> = [
  ...
  testPriority: Array<{ value: string; label: string }> = [
  ...
  addToken() {
    if (
      this.addPatientTestForm.controls['amountPaid']?.value >=
      this.addPatientTestForm.controls['grandTotal'].value
    ) {
      if (
        this.addPatientTestForm.controls['amountPaid']?.value &&
        ...
        console.log(
          'this.addPatientTestForm.value',
          this.addPatientTestForm.value
        );
        this.testService
          .addPatientTest(this.addPatientTestForm.value)
          .pipe(takeUntil(this.componetDestroyed))
          .subscribe({
            next: (x) => {
              console.log(x);
              this.alertService.success(
                'Patient Test add successfully',
                'Success'
              );
            },
            error: (err) => {
              this.alertService.error(
                'Something went wrong while adding patient Test.',
                'Error'
              );
            },
          });
